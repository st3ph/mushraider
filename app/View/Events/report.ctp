    ...
    <h1>
        <div class="row">
            ...
            <div class="span<?php echo $displayAdminButtons?8:11?>">
                <?php echo $this->Html->link('<i class="fa fa-chevron-left"></i>', '/events/view/'.$event['Event']['id'], array('escape' => false));?>
                <?php echo __('Report');?> : <?php echo $event['Event']['title'];?>
            </div>

            <?php if($displayAdminButtons):?>
                <div class="pull-right text-right span3">
                    <?php echo $this->Html->link('<i class="fa fa-pencil-square-o"></i> '.__('Edit'), '/events/close/'.$event['Event']['id'], array('class' => 'btn btn-warning btn-mini', 'escape' => false));?>
                </div>
            <?php endif;?>            
        </div>
    </h1>
    ...
    <?php for($i = 1;$i <= 4;$i++):?>
        <?php if(!empty($report['Report']['screenshot_'.$i])):?>
            <li>
                ...
                <div class="text-center"><small><?php echo $this->Html->link('<i class="fa fa-external-link"></i> '.__('view original'), '/files/reports/'.$report['Report']['screenshot_'.$i].'_o.png', array('target' => '_blank', 'escape' => false));?></small></div>
            </li>
